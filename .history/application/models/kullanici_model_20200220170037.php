...
class kullanici_model extends CI_Model
{
    ...
    public function cevapsayi($sessionid = 0)
    {
        ...
        $this->db->select("s.* , (SELECT s.departman from kullanici where s.departman=kullanici.departman), (SELECT COUNT(c.id) from cevap c where s.id=c.soru_id) as cevapsayi,
