...
class kullanici_model extends CI_Model
{
    ...
    public function cevapsayi($sessionid = 0)
    {
        ...
        $this->db->select("s.* , (SELECT soru.departman from kullanici kd where s.departman=kd.departman), (SELECT COUNT(c.id) from cevap c where s.id=c.soru_id) as cevapsayi,
