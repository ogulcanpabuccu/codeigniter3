...
class kullanici_model extends CI_Model
{
    ...
    public function cevapsayi($sessionid = 0)
    {
        ...
        $this->db->select("s.* , (SELECT COUNT(s.departman) from kullanici kd where s.departman=kd.departman) as departman, (SELECT COUNT(c.id) from cevap c where s.id=c.soru_id) as cevapsayi,
