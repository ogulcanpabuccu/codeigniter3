...
class kullanici_model extends CI_Model
{
    ...
    public function cevapsayi()
    {
        ...
        $this->db->select('s.* , 
    (SELECT COUNT (c.id) from cevap c where s.id=c.soru_id) as cevapsayi,
       (SELECT ct.cevaplama_zamani from cevap ct where s.id=ct.soru_id order by cevaplama_zamani DESC LIMIT 1) as soncevapzamani',false);
        $this->db->from('soru s');
