			...
			<div title="Soru Sor" selected ; data-options="closable:true" style="overflow:auto;padding:20px;display:none;">
					...
					<label>Alıcı Seç : </label> <select id="cc" class="easyui-combobox" name="dept" style="width:100px;">
						...
						<? foreach ($kullanicilar as $kullanici) { ?>
							<? if ($kullanici->kullanici_id != $this->session->kullanici_id) { ?>
								<optio name="alici" value="<? echo $kullanici->kullanici_id; ?>"><? echo $kullanici->kullanici_mail; ?></optio>
							<? } ?>
						<? } ?>
					</select>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-6">
						...
						<input class="easyui-datetimebox" name="birthday" data-options="required:true,showSeconds:false" value="<?php echo date('d.m.Y H:i:s'); ?>" style="width:150px">
