		...
		$(function() {
			...
			$.cevapkaydet = function() {
				$.ajax({
					...
					dataType: 'json',
					refresh: true,
					data: $('#Formcevap<? echo $sorudetay->id; ?>').serialize(),
					success: function(gelenveri) {
						if (gelenveri.success) {
							$("#cevapsonuc<? echo $sorudetay->id; ?>").html(gelenveri.hataMesaji);
							$("#cevapdetay<? echo $sorudetay->id; ?>").val("");
							


							var tab<? echo $sorudetay->id; ?> = $('#tt').tabs('getSelected');
var index<? echo $sorudetay->id; ?> = $('#tt').tabs('getTabIndex',$('#tt').tabs('getSelected'));
alert(index<? echo $sorudetay->id; ?>);


							//var p = $('#tt').tabs('getTabIndex', $('#tt').tabs('getTabIndex',$('#tt').tabs('getSelected'))); // get the first tab panel
							//p.panel('refresh');
